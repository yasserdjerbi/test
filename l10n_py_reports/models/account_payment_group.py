...
# Falta migrar el modulo account_withholding, account_withholding_automatic
        ...
        if checks or payments:
            ...
            for check in checks:
                print('Cheque nro %s - %s - Venc. %s' % (check.name, check.bank_id.name or check.journal_id.name, check.payment_date)) # noqa
            ...
            for pay in payments:
                print('%s%s' % (pay.journal_id.name, pay.other_currency and ' (%s %s)' % (pay.signed_amount, pay.currency_id.name) or '')) # noqa

# #############################################################################
        documents = self.with_context(payment_group_id=self.id).matched_move_line_ids # noqa
